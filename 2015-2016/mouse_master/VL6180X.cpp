...
uint8_t VL6180X::readRangeTriple()
{
  writeReg(SYSRANGE__START, 0x01);
  int reading1 = readRangeContinuous();
  int reading2 = readRangeContinuous();
  int min = min(reading1, reading2);
  int max = max(reading1, reading2);
  int reading3 = readRangeContinuous();
  if(reading3 < min) {
    return min;
  }
  if(reading3 > max) {
