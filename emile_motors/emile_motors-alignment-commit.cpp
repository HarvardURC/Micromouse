...
{
  PIDLeft->SetOutputLimits(-1*MOTOR_SPEED - 5, MOTOR_SPEED + 5);
  PIDRight->SetOutputLimits(-1*MOTOR_SPEED, MOTOR_SPEED);
  PIDLeft->SetTunings(0.6,0.01,0.01);
  PIDRight->SetTunings(0.6,0.01,0.01);
...
void emile_motors::turnLeft()
{
  ...
  //   front_align();
  // }
  if (_rightIR -> readRangeContinuousMillimeters() < (WALL_THRESHOLD - 10)) {
    backFlag = 1;
  }
...
void emile_motors::turnRight()
{
  ...
  //   front_align();
  // }
  if (_leftIR -> readRangeContinuousMillimeters() < (WALL_THRESHOLD - 10)) {
    backFlag = 1;
  }
...
void emile_motors::back_align()
{
  ...
  moveTicks(-TICKS_CELL / 1.9, -TICKS_CELL / 1.9);
  delay(100);
...
void emile_motors::followTicksRight(int ticks)
{
  ...
  PIDRight->SetTunings(0.6,0.01,0.01); // determine tunings
  // follow until combined motor ticks double the ticks parameter
  while (distanceL + distanceR < ticks * 2 && _leftIR->readRangeContinuousMillimeters() < WALL_THRESHOLD + 20 &&
         _rightIR->readRangeContinuousMillimeters() < WALL_THRESHOLD - 50)
  {
    distanceL = encoderLeft->read();
    distanceR = encoderRight->read();
    InputR = _rightIR->readRangeContinuousMillimeters(); //read right IR sensor
    InputL = _leftIR->readRangeContinuousMillimeters();
    SetpointR = WALL_DISTANCE_RIGHT;
    PIDRight->Compute();
    commandMotors(MOTOR_SPEED + 0.3 * (OutputR - (InputL - InputR)), MOTOR_SPEED + 0.3 * OutputR);
  }
  int off = ticks - (distanceL + distanceR) / 2;
  moveTicks(off, off);
}
...
void emile_motors::followTicksLeft(int ticks)
{
  ...
  // follow until combined motor ticks double the ticks parameter
  while (distanceL + distanceR < ticks * 2 && _rightIR->readRangeContinuousMillimeters() < WALL_THRESHOLD + 30 &&
         _leftIR->readRangeContinuousMillimeters() < WALL_THRESHOLD - 50)
  {
    distanceL = encoderLeft->read();
    distanceR = encoderRight->read();
    InputR = _rightIR->readRangeContinuousMillimeters(); //read right IR sensor
    ...
    PIDLeft->Compute();
    commandMotors(MOTOR_SPEED + 0.2 * OutputL, MOTOR_SPEED + 0.2 * (OutputL - (InputR - InputL)));
...
void emile_motors::advance()
{
  // use right wall if available
  if (_rightIR->readRangeContinuousMillimeters() < WALL_THRESHOLD - 50){
    followTicksRight(TICKS_CELL);
  }
  // otherwise, use left wall if available
  else if (_leftIR->readRangeContinuousMillimeters() < WALL_THRESHOLD - 50){
    followTicksLeft(TICKS_CELL);
  }
  // if not able to wall follow, use odometry
  else{
    moveTicksNew(TICKS_CELL,TICKS_CELL);
  }
}

// moves left and right motors given # of ticks, maxes at 2 seconds
void emile_motors::moveTicksNew(int Lticks, int Rticks)
{
  if (releaseFlag){
    return;
  }
